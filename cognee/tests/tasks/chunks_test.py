...
def test_chunk_by_sentence_english_and_chinese():
    ...
    english_chunks = chunk_by_sentence.chunk_by_sentence(english_text)
    assert any("sentence" in c for c in english_chunks)
    # Chinese
    chinese_text = "这是一个句子。这里还有另一个！再来一个？"
    chinese_chunks = chunk_by_sentence.chunk_by_sentence(chinese_text)
    assert any("句子" in c for c in chinese_chunks)
